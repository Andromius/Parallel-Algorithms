...
static bool try_load_SRLFP_file(string path_to_file, vector<vector<int>>& out_cost_matrix, vector<int>& out_widths)
{
    ...
    out_cost_matrix = vector<vector<int>>(N, vector<int>(N, 0));
    for (int i = 0; i < N; ++i) {
        for (int j = 0; j < N; ++j) {
            file >> out_cost_matrix[i][j];
        }
    }
    ...
    for (int i = 0; i < N; ++i) {
        for (int j = 0; j < i; ++j) {
            out_cost_matrix[i][j] = out_cost_matrix[j][i];
        }
    }
...
static double calculate_permutation_cost(const vector<vector<int>>& cost_matrix, const vector<int>& permutation, const vector<int>& widths)
{
    double cost = 0;
    for (int i = 0; i < N; ++i)
    {
...
static void find_minimal_permutation_serial_BF(const vector<vector<int>>& cost_matrix, const vector<int>& widths)
{
    ...
    {
        double cost = calculate_permutation_cost(cost_matrix, initial_permutation, widths);
...
static void process(int thread_id, int chunk_size, vector<int> permutation, const vector<vector<int>>& cost_matrix, const vector<int>& widths)
{
    for (int i = 0; i < chunk_size; i++) {
        double cost = calculate_permutation_cost(cost_matrix, permutation, widths);
        if (cost <= global_best_cost)
        {
            mtx.lock();
            global_best_cost = cost;
...
static void find_minimal_permutation_parallel_BF(const vector<vector<int>>& cost_matrix, const vector<int>& widths)
{
	long total_permutations = calculate_total_permutations();
    ...
    auto start = chrono::high_resolution_clock::now();
    workers.push_back(thread(process, 0, chunk_size, initial_permutation, cost_matrix, widths));
	for (int i = 1; i < worker_count; i++)
	{
        ...
        workers.push_back(thread(process, i, chunk_size, initial_permutation, cost_matrix, widths));
	}
...
static void find_minimal_permutation_serial_BnB(const vector<vector<int>>& cost_matrix, const vector<int>& widths)
{
    ...
    for (long i = 0; i < total_permutations; i++)
    {
        ...
        if (cost < 0)
        {
            ...
            while (next_permutation(initial_permutation.begin() + 1,
                initial_permutation.end())) {
                if ((bound2 == -1 && bound != initial_permutation[idx]) ||
                    (bound != initial_permutation[idx] || bound2 != initial_permutation[idx - 1])) {
                    break;
                }
                i++;
            }
...
int main()
{
    ...
    cout << "\033[33mSerial BF\033[0m" << endl;
    global_best_cost = numeric_limits<double>::max();
    find_minimal_permutation_serial_BF(cost_matrix, widths);

    cout << endl << "\033[33mParallel BF\033[0m" << endl;
    global_best_cost = numeric_limits<double>::max();
    find_minimal_permutation_parallel_BF(cost_matrix, widths);

    cout << endl << "\033[33mSerial BnB\033[0m" << endl;
    global_best_cost = numeric_limits<double>::max();
    find_minimal_permutation_serial_BnB(cost_matrix, widths);

    cout << endl << "\033[33mParallel BnB\033[0m" << endl;
    global_best_cost = numeric_limits<double>::max();
